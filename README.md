...
![Tests](https://img.shields.io/badge/tests-366_passing-brightgreen?logo=pytest&logoColor=white)
...
- **Security & Observability**
  - **Role-based access control (ABAC)** — four-phase security layer, all opt-in and disabled by default:
    - *Phase A* — Post-filter on search results by role intersection against `allowed_roles` metadata; deny-by-default when roles omitted; multiplicative overfetch; ACL stripped from responses
    - *Phase B* — JWT middleware (`JWTAuthMiddleware`) extracts and validates Bearer tokens via JWKS; roles derived from configurable JWT claim; fail-closed when JWT enabled; `WWW-Authenticate` headers per RFC 6750
    - *Phase C* — `presign_url()` replaces `s3://` document links with time-limited presigned GetObject URLs (configurable TTL, 15 min default); other schemes passed through; only authorized results trigger S3 calls
    - *Phase D* — Structured audit logging via dedicated `semantic_search.audit` logger; `ac.filter`/`ac.grant`/`ac.auth_failure` events with caller identity; grant logging gated to control volume
  - Configurable via `access_control:`, `presign:`, and `audit:` blocks in `config/app.yaml` or env vars
...
- **feature/doc-link-field — Document Link Field & Access Control:** Added `doc_link`/`doc_name` columns to `sop`/`compliance_procedures` tables; extended `ColumnConfig` with `type: link` and `link_field` for decoupled display text and href rendering; XSS-safe href allowlist; `SERVE_DOCUMENTS` gated static mount. Implemented ABAC Phase A: `AccessControlConfig` with deny-by-default, multiplicative overfetch, and ACL stripping. Test suite: 318 passing.
- **feature/abac-phase-b — JWT Identity Integration:** JWT middleware (`JWTAuthMiddleware`) with JWKS validation, `WWW-Authenticate` headers (RFC 6750), thread-safe eager `PyJWKClient` init, `anyio` thread offload for non-blocking JWKS fetch, fail-closed endpoint guard, RSA/ECDSA/PSS algorithm support, configurable roles claim. Test suite: 332 passing.
- **feature/abac-phase-c — Presigned Document Links:** `presign_url()` utility and `create_presigner()` factory; `PresignConfig` with TTL validation (1s–7d AWS limit); `SearchRuntime` applies presigning after AC filter (only authorized results trigger S3 calls); `doc_link` replaced or removed in response based on presign result. Test suite: 353 passing.
- **feature/abac-phase-d — Audit, Observability & Entitlement Catalog:** `AuditLogger` with structured JSON events (`ac.filter`, `ac.grant`, `ac.auth_failure`) via dedicated `semantic_search.audit` logger; `AuditConfig` with `log_grants` gating; wired into `SearchRuntime` AC post-filter loop. Test suite: 366 passing.
